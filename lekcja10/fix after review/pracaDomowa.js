...
function recommendations(doesHeLikeReadingBooks, doesHeHaveTv) {
    if (doesHeLikeReadingBooks === false || doesHeHaveTv === true) {
        return `Pooglądaj telewizję`
    } else if (doesHeLikeReadingBooks === true) {
        return `Cześć ${personDetailsOld.name} polecam ci przeczytać książkę ${book.title} ma tylko ${book.numberOfPages} stron.`
    } else {
        return `Idź pobiegaj`
    }
}
